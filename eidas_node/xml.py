...
def sign_saml_kms(root: Element):
    """Resign an already signed SAML response using AWS KMS"""
    signature = root.find("./{http://www.w3.org/2000/09/xmldsig#}Signature")
    signaturevalue = signature.find("./{http://www.w3.org/2000/09/xmldsig#}SignatureValue")
    signedinfo = signature.find("./{http://www.w3.org/2000/09/xmldsig#}SignedInfo")

    formated = etree.tostring(signedinfo, method='c14n', exclusive=True)

    client = boto3.client('kms')
    response = client.sign(
        KeyId=CONNECTOR_SETTINGS.service_provider['response_kms_signature'].get('kms_key_alias'),
        Message=bytes(formated),
        MessageType='RAW',
        SigningAlgorithm=CONNECTOR_SETTINGS.service_provider['response_kms_signature'].get('signature_method')
    )

    sig = response['Signature']
    signaturevalue.text = base64.encodebytes(sig)
...
def sign_xml_node(node: Element, key_file: str, cert_file: str,
    ...
    # If  AWS KMS is in use we need to give the proper certificate for XMLSEC too
    kms_enabled = CONNECTOR_SETTINGS.service_provider['response_kms_signature'].get('kms_enabled')
    if kms_enabled:
        cert_file = CONNECTOR_SETTINGS.service_provider['response_kms_signature'].get('cert_file')
    ...
    # If AWS KMS is in use we override the signature generated by XMLSEC
    if kms_enabled:
        sign_saml_kms(node)
